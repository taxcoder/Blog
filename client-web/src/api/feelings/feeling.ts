 ...
 * @LastEditTime: 2025-01-07 08:39:17
...
export const getFeelings = async (index: number, limit: number) => {
  return request<{ allCount: number; list: Array<Feeling> }>(`/api/feelings?index=${index}&limit=${limit}`);
};

export const getFeeling = async (feelingId: number) => {
  return request<Feeling>(`/api/feeling/${feelingId}`);
};
// 给日记点赞
export const addFeelingLikeCount = async (feelingId: number) => {
  return request<Feeling>(`/api/feelings/${feelingId}/like`, {
    method: 'POST'
  }).then((res) => {
