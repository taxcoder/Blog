 ...
 * @LastEditTime: 2025-01-11 08:42:01
...
export const getArticles = async (index: number, limit: number) => {
  return request<Array<ArticleInfo>>(`/api/articles?index=${index}&limit=${limit}`);
};
// 返回哪些年份有上传文章
export const getArticlesCreateTime = async () => {
  return request<Array<String>>(`/api/articles/years`);
};
// 根据年份查询当年所有的文章
export const getArticlesByCreateTime = async (year: string) => {
  return request<Array<Article>>(`/api/articles/createTime/${year}`);
};
...
export const addArticleLikeCount = async (articleId: number) => {
  return request<Article>(`/api/article/${articleId}/like`, {
    method: 'POST'
  }).then((res) => {
