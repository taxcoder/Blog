 ...
 * @LastEditTime: 2025-01-14 06:38:40
 * @FilePath: /code/blog/client-web/src/views/tags/TagInfo.vue
 * @Description: 
...
const tagArticleList = reactive<{ allCount: number; list: any[] }>({
  allCount: -1,
  list: []
});

const fn = (data: any) => {
  tag.setTagArticleList(data);
  global.setFigureName(data.name);
};

const activeFunction = (index: number, limit: number) => {
  let articles = tag.getTagArticleListById(parseInt(route.params.id as string));
  let size = 0;
  // 如果没有存储，则直接请求后台数据，否则从仓库中取数据
  if (!articles || articles.list.length === 0) {
    return api.getArticleByTagId(parseInt(route.params.id as string), index, limit);
  }
  // 获取存储了多少的数据
  if (articles && articles.list.length > 0) {
    // 从仓库中取的数据的长度
...
const array = computed(() => {
  let articles = tag.getTagArticleListById(parseInt(route.params.id as string));
  return articles && articles.list.length > 0 ? articles : tagArticleList;
});
</script>
